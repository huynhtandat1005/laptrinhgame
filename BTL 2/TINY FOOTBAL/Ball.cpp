...
void Ball::update()
{
	...
	else if (velocity.y > 0) /*the ball is going down*/
	{
		if (position.y >= SCREEN_HEIGHT - GROUND_LINE - dstRect.h) /*the ball hits the ground*/
		{
			...
			{
				velocity.y = - SCREEN_HEIGHT / maxHeight + 1; /*rebounding is proportional with the height from which the ball is falling*/
			}
		}
		else
		{
	...
	/*horizontal movement of ball*/
	velocity.x -= velocity.getSignX() * 0.015f; /*horizontal movement of ball is decelerated*/
	if (fabs(velocity.x) < 0.001f) /*the ball stops*/
	{
		velocity.x = 0.0f;
	}
