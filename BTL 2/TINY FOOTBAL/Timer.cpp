...
void Timer::start()
{
	...
	timeText = std::to_string(playing_time);
	textColor = { 255, 255, 255 };
...
bool Timer::update()
{
	if (started && !paused)
	{
		...
		if (currentTime > playing_time)
		{
			return true;
		}
		timeText = std::to_string(playing_time - currentTime);
		if (currentTime > playing_time - 10)
		{
